...
Run multiple fire rounds back-to-back against the same file until it reaches a target count N, rotating source emphasis per round to maximize coverage of the 7-day signal window. Stop early when the target is hit, when signal is exhausted, or when the file hits the global 100-idea ceiling. Refire is a burst, not a schedule — for scheduled mining over time, the user should use Claude Code's `/loop` against `/great-ideas` directly.

The non-negotiable rule: refire does not lower the quality bar to hit N. If a round can't produce 5 strong signal-backed ideas, that's a signal that the domain is exhausted at this moment — stop and report, do not pad.
...
| Round | Emphasis |
|---|---|
| 1 | Product Hunt top of the week + r/SideProject + r/SaaS revenue threads |
| 2 | Hacker News Show HN (>50 points) + IndieHackers revenue milestones + Google Trends rising queries |
| 3 | App Store top charts in scope + domain-specific subreddits + trade press if the domain is a regulated vertical |
| 4 | Platform developer forums (Meta Wearables, Shopify Partners, Stripe, Vercel, OpenAI, Anthropic, Apple, Google, etc, when relevant) + Exploding Topics |
| 5+ | Cycle back to the round-1 sources but bias toward whichever sources produced the highest-validation entries in earlier rounds of this refire |

The standard fire batch-level rules still apply per round: at least 3 distinct verticals, dated source URLs, scope filter enforced, numbering continued from the highest existing entry. Refire does not relax any of these.
...
- One line: exit reason in plain English.
  ...
  - Signal exhausted: "Two consecutive rounds added fewer than 5 strong signal-backed ideas. Domain is exhausted for now — consider widening scope, swapping sources, or coming back in a week when fresh signal lands."
...
- **Padding to hit N.** If a round can't produce 5 strong signal-backed ideas, report thin signal and let the loop terminate. Never relax the dedup rule, the dated-URL rule, or the scope filter to hit a number.
- **Reusing source URLs across rounds.** Each round must surface different posts, threads, charts — that's what the source emphasis rotation is for.
